...
def test_add_unique_player_succeeds_when_adding_first_player():
    ...
    msg = player_manager.add_unique_player("Jei", "Setter", "Beginner")
    assert msg == "✅ Player added"
    assert player_manager.get_player_count() == 1
    assert Player("Jei", "Setter", "Beginner") in player_manager.get_player_list()


def test_add_unique_player_succeeds_when_adding_second_unique_player():
    player_manager = PlayerManager()
    player_manager.add_unique_player("Jei", "Setter", "Beginner")
    msg = player_manager.add_unique_player("Carl", "Setter", "Beginner")
    assert msg == "✅ Player added"
    assert player_manager.get_player_count() == 2
    assert Player("Carl", "Setter", "Beginner") in player_manager.get_player_list()
